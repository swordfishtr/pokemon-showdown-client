...
export class BattleLog {
	...
	add(args: Args, kwArgs?: KWArgs, preempt?: boolean, showTimestamps?: 'minutes' | 'seconds') {
		...
		case 'showteam': {
			...
			const exportedTeam = team.map(set => {
				let buf = Teams.export([set], battle.gtt.dex).replace(/\n/g, '<br />');
	...
	static usernameColor(name: ID) {
		...
		if (Config.customcolors[name]) {
			const data = Config.customcolors[name];
			if(data.startsWith('#')) {
				return this.colorCache[name] = data.slice(0, 7);
			}
			hash = MD5(data);
		} else {
			hash = MD5(name);
		...
		return this.colorCache[name] = `#${toHex(r)}${toHex(g)}${toHex(b)}`;
	}
	...
	static createReplayFile(room: { battle: Battle, id?: string, fragment?: string }) {
		...
		buf += '<!-- version 1 -->\n';
		buf += `<title>${BattleLog.escapeHTML(battle.gtt.format.name)} replay: ${BattleLog.escapeHTML(battle.p1.name)} vs. ${BattleLog.escapeHTML(battle.p2.name)}</title>\n`;
