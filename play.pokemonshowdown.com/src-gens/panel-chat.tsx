...
export class ChatRoom extends PSRoom {
	...
	override clientCommands = this.parseClientCommands({
		'chall,challenge'(target) {
			const [user, format] = PSUtils.splitFirst(target, ',');
			if(!target || this.pmTarget && (user.includes('@@@') || !format)) {
				this.openChallenge(target);
			}
			else {
				PS.getPMRoom(toID(user)).openChallenge(format);
			}
		},
	...
	openChallenge(format?: string) {
		...
		if(this.challenging || this.teamSent) return;
	...
	cancelChallenge() {
		...
		if (this.teamSent || this.challenging) {
			...
			delete this.challengeMenuFormat;
		}
...
export class ChatTextEntry extends preact.Component<{
	...
	override render() {
		...
		if (room.connected === 'client-only' && room.id.startsWith('battle-')) {
			...
			><CopyableURLBox url={`https://replay.generationssd.co.za/${room.id.slice(7)}`} /></div>;
		}
		return <div
			class="chat-log-add hasuserlist" onClick={this.focusIfNoSelection} style={{ left: this.props.left || 0 }}
		>
